...
import { ChevronLeft, ChevronRight, Sparkles, House } from "lucide-react"
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <House className="w-5 h-5 text-yellow-300" />
              <span className="text-xs uppercase tracking-[0.2em] text-yellow-300 font-semibold">Styles</span>
            </div>
            <h2
        ...
        <div className="relative">
          ...
          >
            {data.map((style) => (
              ...
              <div className="group cursor-pointer rounded-2xl overflow-hidden not-md:w-[67dvw] md:w-[25dvw] bg-card shadow-sm hover:shadow-lg transition-all duration-300 hover:scale-[1.02]">
                {/* Hero Image */}
                <div className="relative h-48 md:h-64 overflow-hidden bg-muted">
                  <img
                    src={ style.heroImage }
                    alt={style.title}
                    className="w-full h-full absolute object-cover group-hover:scale-105 transition-transform duration-300"
                  />
                  {/* Dark Overlay */}
